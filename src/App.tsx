...
// --- CONFIGURATION ---
const SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbwiHueRtF1Kpn1ZpP_PdU_5Q6e4uHmh266VTsEOlyJdz1mR7vL4peQW1wOkC9Yz-Tn-/exec";
...
// HAZARD WEIGHTS
const HAZARD_RISK_W = { low: 1, medium: 2, high: 3 };
const HAZARD_STATUS_W = {
  submitted: 1.0,
  accepted: 1.0,
  reassign: 1.0,
  followup: 0.5,
...
const HAZARD_TYPE_W = { tta: 1.5, kta: 1.0 };

// MASTER ADMIN
const MASTER_EMAIL = "fgalangm@gmail.com";
const MASTER_PASS = "AnumanaTrial.2026";
...
const appendSheetData = async (sheetName, dataObj) => {
  try {
    const response = await fetch(`${SCRIPT_URL}?sheet=${sheetName}`, {
      method: "POST",
    ...
    // Membaca balasan dari Google
    const result = await response.json();

    if (result.error) {
      console.error(
        `Error dari Spreadsheet di tab ${sheetName}:`,
        result.error
      );
...
const parseSafeDate = (dateStr) => {
  ...
  // 1. Coba baca langsung (Cocok untuk format ISO dari Google Sheets)
  const directDate = new Date(dateStr);
  if (!isNaN(directDate.getTime())) return directDate;

  // 2. Fallback jika diketik manual (DD-MM-YYYY atau YYYY-MM-DD)
  ...
  if (parts.length >= 3) {
    ...
    // Cek jika formatnya ternyata YYYY-MM-DD
    if (day > 1000) {
      year = day;
    ...
    if (isNaN(month)) {
      ...
      month =
        months[monthStr.substring(0, 3)] !== undefined
          ? months[monthStr.substring(0, 3)]
          : 0;
    }
...
export default function App() {
  ...
  const [sessionEmail, setSessionEmail] = useState(
    localStorage.getItem("anumana_v31_email") || ""
  );
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      try {
        // Ambil data dari 4 sheet secara paralel agar lebih cepat
        ...
        if (sessionEmail) {
          ...
          if (emailLower === MASTER_EMAIL) {
            setUser({ email: emailLower, role: "master-admin" });
          } else {
  ...
  const analytics = useMemo(() => {
    const filteredIncidents =
      selectedArea === "All PIT"
        ? incidents
        : incidents.filter((i) => i.pit === selectedArea);
    const filteredHazards =
      selectedArea === "All PIT"
        ? hazards
        : hazards.filter((h) => h.pit === selectedArea);
    const filteredObs =
      selectedArea === "All PIT"
        ? observations
        : observations.filter((o) => o.pit === selectedArea);

    const sortedForMath = [...filteredIncidents].sort(
      (a, b) =>
        parseSafeDate(b.date).getTime() - parseSafeDate(a.date).getTime()
    );
    ...
    const absoluteDayCount = lastIncident
      ? Math.floor(
          (today - parseSafeDate(lastIncident.date)) / (1000 * 60 * 60 * 24)
        )
      : 0;
    ...
    if (sortedForMath.length > 0) {
      ...
      const variance =
        allGaps.reduce((a, b) => a + Math.pow(b - avgInterval, 2), 0) /
        allGaps.length;
    ...
    const dist = dayNames.map((day) => {
      const dayData = sortedForMath.filter(
        (i) => dayNames[parseSafeDate(i.date).getDay()] === day
      );
      const weighted = dayData.reduce(
        (acc, curr) => acc + (SEVERITY_WEIGHTS[curr.category] || 0),
        0
      );
    ...
    const varianceWeighted =
      dist.reduce((a, b) => a + Math.pow(b.weighted - avgWeighted, 2), 0) / 7;
    ...
    if (sortedForMath.length > 0) {
      ...
      const latestIncidents = sortedForMath.filter(
        (i) => parseSafeDate(i.date).getTime() === lastDateMs
      );
    ...
    const forecast = Array.from({ length: 7 }).map((_, i) => {
      ...
      if (activeObs.length > 0) {
        const zeroCount = activeObs.filter(
          (o) => parseInt(o.temuan || 0) === 0
        ).length;
      ...
      return {
        ...
        penaltyScore: penaltyScore.toFixed(1),
        dayScore,
    ...
    const top5Hazards = Object.entries(locCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);

    const blindSpots = top5Hazards
      .map((haz) => {
        const loc = haz[0];
        const hazCount = haz[1];
        const obsCount = filteredObs.filter(
          (o) => (o.subLokasi || "").toLowerCase() === loc.toLowerCase()
        ).length;
    ...
    filteredObs.forEach((o) => {
      if (parseInt(o.temuan || 0) === 0) {
        pelaporCounts[o.pelapor] = (pelaporCounts[o.pelapor] || 0) + 1;
      }
    });
    const topPasif = Object.entries(pelaporCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);

    return {
      dataCount,
      dayCount: absoluteDayCount,
      avgInterval: parseFloat(avgInterval.toFixed(1)),
      stdDevInterval: parseFloat(stdDevInterval.toFixed(1)),
      maxCycleLength,
      lastIncident,
      forecast,
      dist,
  ...
  const historicalLogs = useMemo(() => {
    let data =
      selectedArea === "All PIT"
        ? incidents
        : incidents.filter((i) => i.pit === selectedArea);
    const hazardDataFiltered =
      selectedArea === "All PIT"
        ? hazards
        : hazards.filter((h) => h.pit === selectedArea);
    const obsDataFiltered =
      selectedArea === "All PIT"
        ? observations
        : observations.filter((o) => o.pit === selectedArea);
    ...
    if (historyMonth !== "All")
      data = data.filter(
        (i) => parseSafeDate(i.date).getMonth() === parseInt(historyMonth)
      );

    const logsWithPreRisk = data.map((log) => {
      const targetTime = parseSafeDate(log.date).getTime();
      const pastLogs = data.filter(
        (p) => parseSafeDate(p.date).getTime() < targetTime
      );

      if (pastLogs.length === 0) return { ...log, preRisk: 5 };

      const lastIncident = pastLogs[0];
      const absoluteDayCount = Math.floor(
        (targetTime - parseSafeDate(lastIncident.date).getTime()) /
          (1000 * 60 * 60 * 24)
      );
      ...
      if (pastLogs.length > 0) {
        ...
        if (pastLogs.length > 1) {
          for (let i = 0; i < pastLogs.length - 1; i++)
            gaps.push(
              Math.abs(
                (parseSafeDate(pastLogs[i].date) -
                  parseSafeDate(pastLogs[i + 1].date)) /
                  (1000 * 60 * 60 * 24)
              )
            );
        }
        ...
        const variance =
          allGaps.reduce((a, b) => a + Math.pow(b - avgInt, 2), 0) /
          allGaps.length;
      ...
      const varW =
        dist.reduce((a, b) => a + Math.pow(b.weighted - avgW, 2), 0) / 7;
      ...
      const latestIncidents = pastLogs.filter(
        (i) => parseSafeDate(i.date).getTime() === lastDateMs
      );
      m2_base = latestIncidents.length > 1 ? 1.0 : 0.0;
      latestIncidents.forEach((inc) => {
  ...
  const handleLogin = (e) => {
    ...
    if (foundUser && foundUser.password === passwordInput) {
      ...
      setAuthError("Email atau password tidak terdaftar.");
    }
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (lines.length < 2) {
          setImportStatus("File CSV kosong.");
          return;
        }
        ...
        if (importType === "hazard") {
          ...
          const idxDept = headers.findIndex(
            (h) =>
              h.includes("departemen pelapor") ||
              h.includes("department pelapor")
          );
          const idxCompany = headers.findIndex((h) =>
            h.includes("perusahaan pelapor")
          );
          const idxType = headers.findIndex((h) => h.includes("jenis temuan"));
          const idxRisk = headers.findIndex(
            (h) => h.includes("resiko") || h.includes("risiko")
          );
          const idxLoc = headers.findIndex(
            (h) => h === "lokasi laporan" || h.includes("lokasi")
          );
          const idxSubLoc = headers.findIndex((h) => h.includes("sub lokasi"));
          const idxStatus = headers.findIndex(
            (h) => h.includes("status laporan") || h === "status"
          );
          const idxDate = headers.findIndex((h) =>
            h.includes("tanggal laporan")
          );
          const idxJudul = headers.findIndex(
            (h) => h.includes("judul laporan") || h.includes("deskripsi")
          );
          ...
          rows.forEach((line) => {
            ...
            if (!company || company.toLowerCase() !== "pt cipta kridatama")
              return;

            const rawDept = cols[idxDept]?.trim();
            let pit = "";
            if (rawDept && rawDept.toLowerCase().includes("girimulya"))
              pit = "GRB";
            else if (rawDept && rawDept.toLowerCase().includes("kusan"))
              pit = "KSB";
            else return;
          ...
          const idxDate = headers.findIndex((h) =>
            h.includes("tanggal observasi")
          );
          const idxSubLoc = headers.findIndex(
            (h) =>
              h === "sublokasi observasi" ||
              h.includes("sub lokasi") ||
              h.includes("sublokasi")
          );
          const idxTemuan = headers.findIndex((h) =>
            h.includes("jumlah temuan")
          );
        ...
        if (validRecords.length > 0) {
          setImportStatus(
            `Menyimpan ${validRecords.length} record ke Spreadsheet (harap tunggu)...`
          );
          ...
          // Memperbarui UI secara instan (optimistic update)
          if (importType === "hazard")
            setHazards((prev) => [...prev, ...validRecords]);
          else if (importType === "observasi")
            setObservations((prev) => [...prev, ...validRecords]);
          else
            setIncidents((prev) =>
              [...prev, ...validRecords].sort(
                (a, b) =>
                  parseSafeDate(b.date).getTime() -
                  parseSafeDate(a.date).getTime()
              )
            );

          // Mengirim ratusan data sekaligus dalam 1x request!
          await appendSheetData(targetSheet, validRecords);

          setImportStatus(
            `Sukses: ${validRecords.length} data tersimpan di Spreadsheet!`
          );
          setTimeout(() => {
  ...
  // CATATAN PENTING UNTUK DELETE DI SPREADSHEET
  const handleDeleteIncident = (id) => {
    if (
      window.confirm(
        "Perhatian: Aplikasi ini hanya menghapus data di layar Anda. Untuk menghapus permanen, Anda harus menghapus barisnya langsung di Google Spreadsheet.\n\nLanjutkan sembunyikan dari layar?"
      )
    ) {
  ...
  const handleAddUser = async () => {
    ...
    if (whitelist.find((u) => u.email === emailLower)) {
  ...
  const handleDeleteUser = (emailToRemove) => {
    if (
      window.confirm(
        "Perhatian: Aplikasi ini hanya menyembunyikan akses sementara. Untuk mencabut akses permanen, hapus baris email ini di tab 'whitelist' Google Spreadsheet.\n\nLanjutkan?"
      )
    ) {
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-slate-950 p-6 font-sans">
        <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl w-full max-w-sm border border-white/5 animate-in zoom-in-95 duration-500">
          ...
          <h2 className="text-3xl font-black text-slate-800 uppercase mb-1 text-center tracking-tighter leading-none">
            Anumana
          </h2>
          <p className="text-slate-400 text-[10px] text-center mb-10 font-black uppercase tracking-widest italic leading-tight text-nowrap">
            Analitik Nuansa & Manajemen Ancaman
          </p>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {authError && (
              <p className="text-rose-500 text-[10px] font-black text-center uppercase leading-tight px-2">
                {authError}
              </p>
            )}
            <button
              type="submit"
              className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl hover:bg-slate-800 transition-all shadow-xl active:scale-[0.98] uppercase tracking-widest text-xs"
            >
              Masuk Dashboard
            </button>
          </form>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {user.role === "master-admin" && (
              <button
                ...
                className="bg-indigo-600 text-white text-[10px] font-black px-5 py-3 rounded-xl flex items-center gap-2 hover:bg-indigo-700 shadow-lg transition-all"
              >
                <Upload className="w-4 h-4" /> CSV DATA
              </button>
            )}
            <button
              onClick={() => setIsEntryModalOpen(true)}
              className="bg-slate-900 text-white text-[10px] font-black px-5 py-3 rounded-xl flex items-center gap-2 hover:bg-slate-800 shadow-lg transition-all active:scale-95"
            >
            ...
            <div className="flex items-center gap-1 ml-2">
              <button
                ...
                className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:text-slate-900 transition-all"
              >
                <User className="w-4 h-4" />
              </button>
              <button
                onClick={handleLogout}
                className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:text-rose-600 transition-all shadow-sm"
              >
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">
                      Live Probabilitas ({selectedArea})
                    </p>
                    <span className="bg-white/10 text-emerald-400 px-3 py-1 rounded-full text-[9px] font-bold tracking-widest uppercase">
                      Total: {analytics.dataCount} Data Log
                    </span>
                  </div>
                  <h2 className="text-8xl font-black tracking-tighter leading-none mt-2">
                    {analytics.forecast[0].prob}%
                  </h2>
                  <div
                    className={`mt-8 inline-flex items-center gap-3 px-6 py-3 rounded-2xl border-2 ${getProbColor(
                      analytics.forecast[0].prob
                    )}`}
                  >
                    <div className="w-3 h-3 rounded-full animate-pulse bg-current"></div>
                    <span className="text-[11px] font-black uppercase tracking-widest">
                      {getProbLabel(analytics.forecast[0].prob)} LEVEL
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-6xl font-black text-slate-800 tracking-tighter leading-none">
                    {analytics.dayCount}{" "}
                ...
                <div className="mt-4 border-t pt-6 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
                  ...
                  <span className="text-rose-600 font-bold text-sm">
                    #
                    {Math.floor(analytics.dayCount / analytics.maxCycleLength) +
                      1}
                  </span>
                  <br />
                  Hari Efektif Siklus:{" "}
                  <span className="text-slate-900 font-bold">
                    {analytics.dayCount % analytics.maxCycleLength} /{" "}
                    {analytics.maxCycleLength}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col justify-between overflow-hidden relative">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                    Rata-Rata Area ({selectedArea})
                  </p>
                  <h3 className="text-6xl font-black text-slate-800 tracking-tighter leading-none">
                    {analytics.avgInterval || 0}{" "}
                    <span className="text-xl text-slate-300">d</span>
                  </h3>
                </div>
                <div className="mt-4 border-t pt-6 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
                  Standar Deviasi:{" "}
                  <span className="text-indigo-600 font-bold">
                    ±{analytics.stdDevInterval} Hari
                  </span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                    <EyeOff className="w-4 h-4 text-slate-900" /> Titik Buta
                    Pengawasan (Blind Spots)
                  </h3>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {analytics.blindSpots.length > 0 ? (
                        analytics.blindSpots.map((spot, idx) => (
                          ...
                          >
                            <td className="px-4 py-3 text-xs font-bold text-slate-700 truncate max-w-[150px]">
                              {spot.loc}
                            </td>
                        ...
                        <tr>
                          <td
                            colSpan="3"
                            className="px-4 py-8 text-center text-xs text-slate-400 font-bold"
                          >
                            Belum ada data Blind Spot di {selectedArea}.
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 text-[9px] font-medium text-slate-400 uppercase tracking-widest text-center">
                  Sub-lokasi di atas memiliki Hazard Tinggi namun diabaikan
                  pengawas.
                </div>
              </div>
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                    <UserX className="w-4 h-4 text-slate-900" /> Top 5 Pelapor
                    Pasif (0 Temuan)
                  </h3>
                </div>
                <div className="flex-1 overflow-x-auto">
                      ...
                      {analytics.topPasif.length > 0 ? (
                        ...
                        <tr>
                          <td
                            ...
                            className="px-4 py-8 text-center text-xs text-slate-400 font-bold"
                          >
                            Belum ada data pelapor pasif di {selectedArea}.
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 text-[9px] font-medium text-slate-400 uppercase tracking-widest text-center">
                  Data ini menjadi penyumbang Penalty m5 (Kualitas Pengawasan).
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col items-center">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2 w-full">
                  <AlertTriangle className="w-4 h-4 text-amber-500" /> Rasio
                  Hazard KTA vs TTA
                </h3>
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          {
                            name: "KTA (Kondisi)",
                            value: analytics.hazardStats.ktaCount,
                          },
                          {
                            name: "TTA (Tindakan)",
                            value: analytics.hazardStats.ttaCount,
                          },
                        ]}
                        cx="50%"
                        cy="50%"
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          {
                            name: "Open (High Risk)",
                            value: analytics.hazardStats.statOpen,
                          },
                          {
                            name: "Followup",
                            value: analytics.hazardStats.statFollow,
                          },
                          {
                            name: "Closed (END)",
                            value: analytics.hazardStats.statEnd,
                          },
                        ]}
                        cx="50%"
                        cy="50%"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 h-96 flex flex-col">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  <div>
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                      <BarChart3 className="w-4 h-4 text-slate-900" /> Bobot
                      Risiko Area ({selectedArea})
                    </h3>
                  </div>
                </div>
                <div className="flex-1 min-h-0">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Bar
                        dataKey="weighted"
                        name="Total Bobot Insiden"
                        radius={[8, 8, 8, 8]}
                        barSize={40}
                      >
                        {analytics.dist.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={getBarColor(
                              entry.weighted,
                              analytics.avgWeighted,
                              analytics.redThreshold
                            )}
                          />
                        ))}
                      </Bar>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 text-[10px] font-medium text-slate-400 leading-relaxed text-center sm:text-left">
                  <span className="font-bold text-slate-700">
                    Formula Ambang Batas {selectedArea}:
                  </span>
                  <br />
                  Avg ({analytics.avgWeighted.toFixed(0)}) + StdDev (
                  {analytics.stdDevWeighted.toFixed(0)}) ={" "}
                  <strong className="text-rose-600">
                    Batas Anomali: {analytics.redThreshold.toFixed(0)}
                  </strong>
                </div>
              </div>
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 h-96 flex flex-col">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-slate-900" /> Prediksi 7
                  Hari (Integrated 5-Pillar)
                </h3>
                <div className="flex-1 min-h-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 text-[10px] font-medium text-slate-400 leading-relaxed text-center sm:text-left">
                  Sistem 5 Pilar: Exposure Day (m1), Anomali Insiden (m2),
                  Category Incident (m3), Hazard Status Open (m4), dan
                  Pelaksanaan Observasi (m5) .
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden mb-8">
              <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center bg-slate-50/50 gap-6">
                <h3 className="font-black text-slate-800 uppercase tracking-tight text-sm">
                  Matrix Forecasting : Area {selectedArea}
                </h3>
                <div className="flex items-center gap-2 px-4 py-1.5 bg-white border border-slate-100 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-widest shadow-sm">
                  <Activity className="w-3 h-3 text-emerald-500" /> Integrated
                  Risk Index (5 Parameters)
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-white text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                    <tr>
                      <th className="px-10 py-6">Hari / Tanggal</th>
                      <th className="px-10 py-6">Status Risiko</th>
                      <th className="px-10 py-6">Probabilitas Akhir</th>
                      <th className="px-10 py-6">
                        Detail Parameter (m1 - m5) & Sigma
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {analytics.forecast.map((row, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-10 py-6">
                            <span
                              className={`px-4 py-1.5 rounded-full text-[10px] font-black border uppercase ${style}`}
                            >
                              {label}
                            </span>
                          </td>
                          <td className="px-10 py-6">
                            <div className="flex items-center gap-4">
                              <span className="font-black text-slate-800 text-xl w-12">
                                {row.prob}%
                              </span>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden mb-12">
              <div
                className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center bg-slate-50/50 gap-6 cursor-pointer hover:bg-slate-100/50 transition-colors"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="font-black text-slate-800 uppercase tracking-tight text-sm">
                    Log Insiden Historikal ({selectedArea})
                  </h3>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-3 py-1 bg-white border border-slate-200 rounded-full">
                    {historicalLogs.length} Data Terbaca
                  </span>
                  {isHistoryOpen ? (
              ...
              {isHistoryOpen && (
                <div className="p-8 animate-in slide-in-from-top-4 duration-300">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
                    ...
                    <select
                      className="bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold rounded-xl px-4 py-2.5 outline-none focus:border-slate-400 focus:bg-white transition-all cursor-pointer"
                      value={historyMonth}
                    ...
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                        <tr>
                          ...
                          <th className="px-6 py-4">Prediksi Pra-Insiden</th>
                          <th className="px-6 py-4">Deskripsi Insiden</th>
                          <th className="px-6 py-4 text-center">PIT Area</th>
                        ...
                        {historicalLogs.length > 0 ? (
                          historicalLogs.map((log) => {
                            ...
                            return (
                              ...
                              >
                                <td className="px-6 py-5 text-xs font-bold text-slate-700 whitespace-nowrap">
                                  {log.date}
                                </td>
                                <td className="px-6 py-5">
                                  <span
                                    className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest ${
                                      ["Fatality", "LTI"].includes(log.category)
                                        ? "bg-rose-100 text-rose-600"
                                        : ["RWDI", "MTC"].includes(log.category)
                                ...
                                <td className="px-6 py-5">
                                  <span
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${preStyle}`}
                                  >
                                    {log.preRisk}% ({preLabel})
                                  </span>
                                </td>
                                <td
                                  className="px-6 py-5 text-xs font-medium text-slate-600 min-w-[200px] max-w-sm truncate"
                                  title={log.judul}
                                >
                                  {log.judul}
                                </td>
                                <td className="px-6 py-5 text-center">
                                  <span className="px-3 py-1 bg-white border border-slate-200 rounded-md text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    {log.pit}
                                  </span>
                                </td>
                            ...
                            <td colSpan="6" className="px-6 py-12 text-center">
                              <div className="flex flex-col items-center justify-center text-slate-400">
                                ...
                                <span className="text-xs font-bold uppercase tracking-widest">
                                  Tidak ada insiden tercatat pada filter ini.
                                </span>
                              </div>
                            </td>
                          </tr>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {whitelist.map((item, index) => (
                <div
                  key={index}
                  className="bg-slate-50 p-8 rounded-[2rem] flex items-center justify-between shadow-sm group hover:border-slate-300 transition-all border border-transparent"
                >
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-10 text-center shadow-2xl animate-in zoom-in-95">
            ...
            <h3 className="text-2xl font-black uppercase text-slate-800 mb-2">
              Import CSV Data
            </h3>
            <p className="text-xs text-slate-400 mb-6 font-medium italic">
              Pilih jenis data yang akan diunggah
            </p>

            <div className="flex justify-center gap-3 mb-8 flex-wrap">
              <label
                className={`cursor-pointer flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-4 py-3 rounded-2xl transition-all ${
                  importType === "incident"
              ...
              >
                <input
                  ...
                  name="importType"
                  value="incident"
                  checked={importType === "incident"}
                  onChange={() => setImportType("incident")}
                  className="hidden"
                />
                Insiden
              </label>
              ...
              >
                <input
                  ...
                  name="importType"
                  value="hazard"
                  checked={importType === "hazard"}
                  onChange={() => setImportType("hazard")}
                  className="hidden"
                />
                Hazard
              </label>
              ...
              >
                <input
                  ...
                  name="importType"
                  value="observasi"
                  checked={importType === "observasi"}
                  onChange={() => setImportType("observasi")}
                  className="hidden"
                />
                Observasi
              </label>
            ...
            <div className="relative border-4 border-dashed border-slate-100 rounded-[2.5rem] p-12 hover:border-indigo-400 bg-slate-50 group transition-all">
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                {importStatus || "Pilih File .CSV Anda"}
              </p>
            </div>
            <button
              onClick={() => {
              ...
              className="mt-10 text-[10px] font-black text-slate-400 uppercase hover:text-rose-600 transition-colors tracking-widest outline-none"
            >
              Tutup Panel
            </button>
          </div>
            ...
            <form onSubmit={submitManual} className="p-10 space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  name="judul"
                  required
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold outline-none focus:ring-2 focus:ring-slate-900 transition-all"
                />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select
                    name="pit"
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold outline-none"
                  >
                    <option>KSB</option>
                    <option>GRB</option>
                  </select>
              ...
              <button
                type="submit"
                className="w-full bg-slate-900 text-white font-black py-6 rounded-[2rem] shadow-xl uppercase tracking-widest outline-none hover:bg-slate-800 transition-all active:scale-95"
              >
                Simpan Data
              </button>
            </form>
